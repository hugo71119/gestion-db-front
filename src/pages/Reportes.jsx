              ...
              <tbody>
                {auditoria.map(a => (
                  <tr key={a.auditoria_id} className="hover:bg-slate-50/70">
                    ...
                    <td className={`${TD} text-slate-500`}>{a.fecha ? new Date(a.fecha).toLocaleDateString('es-MX', {day:'2-digit', month:'short', year:'numeric'}) : '-'}</td>
              ...
              <tbody>
                {errores.map(e => (
                  <tr key={e.error_id} className="hover:bg-slate-50/70">
                    ...
                    <td className={`${TD} text-slate-500`}>{e.fecha ? new Date(e.fecha).toLocaleDateString('es-MX', {day:'2-digit', month:'short', year:'numeric'}) : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
